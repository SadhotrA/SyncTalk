...
const LoginPage = () => {
  ...
  const { login, verify2FALogin, isLoggingIn, tempUser } = useAuthStore();

  const requires2FA = !!tempUser;

  const handleSubmit = async (e) => {
    e.preventDefault();
    if (requires2FA) {
      try {
        await verify2FALogin(twoFAToken);
      } catch {
        // error handled in verify2FALogin
      }
    } else {
      login(formData);
    }
        ...
        <div className="w-full max-w-md space-y-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            {!requires2FA ? (
              ...
              <div className="form-control">
                <label className="label">
                  <span className="label-text font-medium">Authenticator Code</span>
                </label>
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                  ...
                  <input
                    type="text"
                    className="input input-bordered w-full pl-10 text-center tracking-[0.5em] text-lg"
                    placeholder="000000"
                    maxLength={6}
                    value={twoFAToken}
